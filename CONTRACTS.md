...
**Scaffolded, blocked on Dev's model.** NestJS (`backend/src/prediction/safety-risk.controller.ts`) and FastAPI (`ml-service/main.py`'s `predict_safety_risk`) are both fully built and verified end-to-end — currently always returns `source: "fallback_unavailable"` since no model file exists yet at `data-ml/model/safety_risk_model.joblib`. Once Dev drops that file in with the expected shape (documented in `predict_safety_risk`'s docstring in `main.py`), the endpoint should work with **no NestJS changes and no FastAPI route changes** — only the `topFactors` contribution calculation inside that one function may need adjusting depending on the model's preprocessing shape.
