...
**Status: build complete.** Every endpoint below is implemented and verified (see each section's status line and the Change Log at the bottom) — including §4.1's Composite Safety Risk Score, the last piece to land. This file remains the single source of truth for exact shapes/thresholds; do not change unilaterally — flag changes to the other two before editing, and log them below.
...
**Implemented and verified end-to-end.** Dev trained a logistic regression pipeline (`data-ml/train_safety_risk.py`, model saved to `data-ml/model/safety_risk_model.joblib`) and updated `ml-service/main.py`'s `predict_safety_risk` to compute real `topFactors` from the pipeline's transformed coefficients × feature values (not placeholders), plus tuned `riskTier` thresholds (`<0.2565 low`, `0.2565–0.5532 medium`, `>0.5532 high`) against the real score distribution. Verified live: `source: "model"` with sensible scores for both a clearly-risky input (unfastened seatbelt, close proximity, high idling → `riskScore: 0.6`, `high`) and a clearly-safe one (`riskScore: 0.09`, `low`).
...
```
NestJS's `/health` pings FastAPI's `/health` with a 1-second timeout and reports `pythonServiceReachable`. This same cached signal is what `/predict-task-time` and `/predict-safety-risk` consult as an actual circuit breaker (§8.5, fully implemented) — see `MlServiceHealthService`.
...
```
`status` bands, matching the Operator Score's convention exactly: `EXCELLENT` 85-100, `GOOD` 65-84, `NEEDS_ATTENTION` 40-64, `CRITICAL` 0-39. `componentScores` sum to `score`; final weights (`wearUsageLoad` 25, `fuelEfficiencyDrift` 20, `idlingBurden` 15, `incidentAssociation` 30, `serviceIntervalProximity` 10 — see "Additional Thresholds" below) do weight `incidentAssociation` highest, as anticipated.
...
| — | **Scaffolded §4.1 `/predict-safety-risk` end-to-end** ahead of Dev's model, so his remaining work is training-only. Built `backend/src/prediction/safety-risk.controller.ts` (mirrors `prediction.controller.ts`'s circuit-breaker/logging pattern, but per §4.1 has no local fallback formula — always surfaces `fallback_unavailable` instead of fabricating a score) and `ml-service/main.py`'s `predict_safety_risk` route (loads `data-ml/model/safety_risk_model.joblib` if present, else returns the same `fallback_unavailable` shape). Verified end-to-end (NestJS → FastAPI) with no model file present — correctly returns `fallback_unavailable` today. Expected model shape (features, column names/order, `.predict_proba` usage, `topFactors` calculation) documented directly in `predict_safety_risk`'s docstring for Dev to read on handoff — no separate doc needed. | Ripun |
| — | **§4.1 fully implemented.** Cherry-picked Dev's `data-ml/train_safety_risk.py` + trained model (`safety_risk_model.joblib`) + his real `topFactors` implementation in `ml-service/main.py` (his branch predated Anamika's frontend merge, so a normal merge would have deleted it — cherry-picked just his commit onto `main` instead, verified frontend untouched). **Bug found and fixed during integration:** his commit had changed the ML service URL from port `8001` to `8000` across 3 NestJS files (`ml-service-health.service.ts`, `prediction.controller.ts`, `safety-risk.controller.ts`), silently breaking against the locked port in this file's own header — reverted to `8001` in all three. Verified end-to-end after the fix: both `/predict-task-time` and `/predict-safety-risk` return `source: "model"` with sensible outputs. | Ripun, after integrating Dev's model |
| — | **Full documentation sync pass across README.md, EXECUTION_PLAN.md, and this file**, now that the build itself is complete. Added a top-level "Current Build Status" summary to README.md §0 and status markers on every §7 differentiator (§7.1–§7.6) and §7.7 (production hardening), all now ✅. Added the equivalent status markers to EXECUTION_PLAN.md §6.2/§6.4/§6.5/§6.6 and §7, which had drifted furthest from reality (still read "not yet started"/"blocked on Dev" for features that had been done for hours). Fixed two remaining stale notes in this file: §10's circuit-breaker note (said "not yet wired," contradicted §8.5 being marked fully implemented elsewhere) and §11's "exact weights TBD by Dev" note (weights have been finalized and live since the machine-health merge). This entry documents the doc sync itself, not a code change. | Ripun |
